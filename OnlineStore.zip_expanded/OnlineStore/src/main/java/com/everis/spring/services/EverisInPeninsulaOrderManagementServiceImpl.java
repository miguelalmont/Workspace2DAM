...
/**
 * Servicio de gestión de pedidos dentro de la península de la tienda Everis.
 * 
 * @author malcanmo
 *
 */
@Service("inPeninsula")
public class EverisInPeninsulaOrderManagementServiceImpl implements EverisOrdersManagementServiceI {

	// Declaración del pedido para dentro de la península.
	...
	public EverisOrder createOrder(String orderId, String orderDestinationName, String orderAddress) {
		...
		order = new EverisOrder(orderId, orderDestinationName, orderAddress, Boolean.FALSE);
	...
	public EverisProduct calculatePvpProductByID(final Integer productId) {
		...
		// Comprueba que el envío sea nacional dentro de la península.
		if (!order.getOrderOutOfPeninsula()) {

			// Recupera el producto buscado.
			searchedProduct = order.searchProducByID(productId);

			// Aplicamos el IVA (impuesto del 21% sobre el precio)
			searchedProduct
					.setProductPVP(searchedProduct.getProductPrice() + (searchedProduct.getProductPrice() * 0.21));
		}
